...
class interaction_yarp(yarp.RFModule):
    ...
    def configure(self, rf):
        ...
        """
        string_command = 'from SAM.SAM_Drivers import ' + self.args[4] + ' as Driver'
        exec string_command
        ...
        if self.modelRoot in parser2.sections():
            ...
            self.mm = initialiseModels([self.dataPath, self.modelPath, self.driverName], update='update',
                                       config=self.model_config, initMode='interaction', drawLatent=self.drawLatent)
            ...
            if self.inputType not in ['imagergb', 'imagemono', 'bottle']:
                raise Exception(
    ...
    def read_frame(self):
        ...
        if self.inputType == 'bottle':
            return frame_read.toString()
        elif 'image' in self.inputType:
            return yarp_image_to_np(frame_read, image_type=self.inputType)
